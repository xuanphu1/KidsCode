...
Blockly.Arduino.AirsenseReadAnalog = function () {
  var pin_analog = this.getFieldValue('Analog_Pin');
  Blockly.Arduino.addSetup('Set input analog pin ' + pin_analog, 'pinMode(' + pin_analog + ',INPUT);', false);
  Blockly.Arduino.addDeclaration('Function read analog', `
  ...
  var code = 'readAnalog(' + pin_analog + ')';
  return [code, Blockly.Arduino.ORDER_ADDITIVE];
}


Blockly.Arduino.AirsenseWriteAnalog = function () {
  var pin_analog = this.getFieldValue('Analog_Pin');
  var value = this.getFieldValue('Value');
  Blockly.Arduino.addSetup('Set output analog pin ' + pin_analog, 'pinMode(' + pin_analog + ',OUTPUT);', false);
  var code = 'analogWrite(' + pin_analog + ',' + value + ');\n';
  return code;
}

Blockly.Arduino.AirsenseWriteDigital = function () {
  var pin_digital = this.getFieldValue('Digital_Pin');
  var value = this.getFieldValue('Value');
  Blockly.Arduino.addSetup('Set output digital pin ' + pin_digital, 'pinMode(' + pin_digital + ',OUTPUT);', false);
  var code = 'digitalWrite(' + pin_digital + ',' + value + ');\n';
  return code;
}

Blockly.Arduino.AirsenseReadDigital = function () {
  var pin_digital = this.getFieldValue('Digital_Pin');
  Blockly.Arduino.addSetup('Set input digital pin ' + pin_digital, 'pinMode(' + pin_digital + ',INPUT);', false);
  Blockly.Arduino.addDeclaration('Function read digital', `
    int readDigital(int pin){
      return digitalRead(pin);
    }`
  )

  var code = 'readDigital(' + pin_digital + ')';
  return [code, Blockly.Arduino.ORDER_ADDITIVE];
}
